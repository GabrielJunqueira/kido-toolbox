...
app.include_router(geojson_validator.router)
...
@app.get("/tourism-report")
async def tourism_report_page():
    """Serve the AOI Tourism Report page."""
    page_path = os.path.join(FRONTEND_DIR, "pages", "tourism_report.html")
    if os.path.exists(page_path):
        return FileResponse(page_path)
    return {"error": "Page not found"}
...
@app.get("/geojson-validator")
async def geojson_validator_page():
    """Serve the GeoJSON Validator tool page."""
    page_path = os.path.join(FRONTEND_DIR, "pages", "geojson_validator.html")
    if os.path.exists(page_path):
        return FileResponse(page_path)
    return {"error": "Page not found"}
